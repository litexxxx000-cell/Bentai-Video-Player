...
from Bentai.util.file_properties import get_name, get_hash, get_media_file_size
...
async def start(client, message):
    if not await checkdb.is_user_exist(message.from_user.id):
        ...
        name = await client.ask(message.chat.id, "<b>Welcome To Bentai Disk.\n\nIts Time To Create Account On Bentai Disk\n\nNow Send Me Your Business Name Which Show On Website\nEx :- <code>Tech Bentai</code></b>")
        if name.text:
        ...
        rm = InlineKeyboardMarkup([[InlineKeyboardButton("✨ Update Channel", url="https://t.me/Bentai_Disk")]])
...
async def stream_start(client, message):
    ...
    encoded_url = f"{LINK_URL}?Bentai={link}"
...
async def link_start(client, message):
    if not message.text.startswith(LINK_URL):
        return
    link_part = message.text[len(LINK_URL + "?Bentai="):].strip()
    try:
