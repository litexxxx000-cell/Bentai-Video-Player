...
from Bentai.util.human_readable import humanbytes
from Bentai.server.exceptions import InvalidHash
from pyrogram.errors import FloodWait
from Bentai.util.file_properties import get_name, get_hash, get_media_file_size, get_file_ids
...
async def render_page(id, user, secid, thid, src=None):
    ...
    if id != 0:
        try:
            fileone = await BentaiBot.get_messages(int(LOG_CHANNEL), int(id))
        except:
            fileone = await BentaiBackUpBot.get_messages(int(LOG_CHANNEL), int(id))
    ...
    if secid != 0:
        try:
            filetwo = await BentaiBot.get_messages(int(LOG_CHANNEL), int(secid))
        except:
            filetwo = await BentaiBackUpBot.get_messages(int(LOG_CHANNEL), int(secid))
    ...
    if thid != 0:
        try:
            filethree = await BentaiBot.get_messages(int(LOG_CHANNEL), int(thid))
        except:
            filethree = await BentaiBackUpBot.get_messages(int(LOG_CHANNEL), int(thid))
    ...
    if tag in ["document", "video", "audio"]:
        template_file = "Bentai/template/req.html"
    else:
        template_file = "Bentai/template/dl.html"
